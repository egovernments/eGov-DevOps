...
for schema in $ulb_list
do
        IFS=: read schema_name DUMP <<< $schema
	if [[  -z ${schema_name} && -z ${DUMP} ]]
	then
...
EOF
		pg_restore  $DUMP | sed -e "s/public/${PREFIX}${schema_name}/g" | psql -h localhost -U $USERNAME --no-password -p 5432 -d $DATABASE -n ${PREFIX}${schema_name}
else
